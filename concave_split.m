...
for i = id     
    ...
    while 0 < 1
        ...
        for m = 1:min([length(split_lines),4])
        ...
        if length(e_error) == 2 && e_error(1) < ee_thresh && e_error(2)< ee_thresh 
...
%             end
            ...
            object_new(sub2ind(size(object),split_lines{final_error_array(1)}(:,1),split_lines{final_error_array(1)}(:,2))) = 0;
            object_new2 = bwlabel(object_new,4);
            
            object_composite = object_new2;

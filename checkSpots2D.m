...
for i = 1:length(cell_region_struct_2d)
    all_cell_ids = [all_cell_ids cell_region_struct_2d(i).Cell];
    perma_cell_ids = [perma_cell_ids cell_region_struct_2d(i).Cell];
end

for j = 7:-1:1%length(perma_cell_ids):-1:1
    
    one_cell_id = cell_region_struct_2d(j).Cell;
    ...
    for si = 1:length(cell_struct(one_cell_id).Spots(:,1))
        ...
        spot_z = 130*abs(cell_struct(one_cell_id).Spots(si,4));
        
        
        if spot_z > middle_section_cutoff/2
            %cell_struct(one_cell_id).Spots(spot_id,5) = 0;
            continue
        end

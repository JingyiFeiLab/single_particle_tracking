...
strain_i = strcat(['MR',num2str(strain)]);
...
dateDir = strcat([strainDir,'/',Date]);
if exist(dateDir,'dir')~=7
    mkdir(strainDir,Date)
end

t = strcat(['t',num2str(time)]);
...
filename = strcat([dateDir,'/t',num2str(time),'/sample_00',num2str(sample),color,'_3D','.mat']);
...
%for j = 55:-1:1    
    ...
    hold on
    if sum(cell_region==3) ~= 0
        gscatter(cell_center_x,cell_center_y,cell_region,'kgbm','.',10,'filled');grid on
    else
        gscatter(cell_center_x,cell_center_y,cell_region,'kgm','.',10,'filled');grid on
    end
    
    %set(gcf,'position',[243,868,868,667])
    set(gcf,'position',[1,248,512,384])
    prompt={'Keep Cell(1 == Yes && 2== NO) ?'};   % A box will take in the values for the X/Ytranslation

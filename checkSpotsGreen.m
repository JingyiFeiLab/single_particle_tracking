...
for j = length(perma_cell_ids):-1:1
    
    one_cell_id = cell_region_struct(j).Cell;
    ...
    for si = 1:length(cell_struct(one_cell_id).Green_Spots(:,1))
        spot_id = cell_struct(one_cell_id).Green_Spots(si,1);
        ...
        if cell_struct(one_cell_id).Green_Spots(si,5) == 0
            continue
        end
        
        from_pole = abs(cell_struct(one_cell_id).Green_Spots(si,3));
        if (.5*cell_struct(one_cell_id).Cell_Y_Axis+2-from_pole)/(.5*cell_struct(one_cell_id).Cell_Y_Axis) > -5
            
            cell_center_x = [cell_center_x cell_struct(one_cell_id).Green_Spots(si,2)];
            cell_center_y = [cell_center_y cell_struct(one_cell_id).Green_Spots(si,3)];
            cell_center_x2d = [cell_center_x2d spot_row];
            cell_center_y2d = [cell_center_y2d spot_col];
            cell_center_z = [cell_center_z cell_struct(one_cell_id).Green_Spots(si,4)];
            cell_region = [cell_region cell_struct(one_cell_id).Green_Spots(si,5)];
            if cell_struct(one_cell_id).Green_Spots(si,5) == 4
                cell_color = [cell_color; [1 0 1]];
            elseif cell_struct(one_cell_id).Green_Spots(si,5) == 3
                cell_color = [cell_color; [0 0 1]];
            elseif cell_struct(one_cell_id).Green_Spots(si,5) == 1
                cell_color = [cell_color; [0 0 0]];
            elseif cell_struct(one_cell_id).Green_Spots(si,5) == 2

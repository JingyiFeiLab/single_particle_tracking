...
shape2D_thresh = 8;  % <--- Splitting threshold, you can change this % higher is more generous   (default=5.5)

background_thresh = .1; % lower is generous (default=.1~.2)
pix_neigh =5; %floor((.08/pix_size)*12); <- If you have no idea, try this. Default=8 or 11

low_pass_thresh = 0.01; % less is more generous (try 0.01 or 0.02 )
cell_area_thresh =1.3; % less is more generous (around 1.3)
...
BW = imfill(smallID(bwlabel(BW,4)),'holes');

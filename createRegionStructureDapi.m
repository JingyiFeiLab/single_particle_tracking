...
for ri = 1:length(cell_struct)
    ...
    strcat(['Constructing Cell ' num2str(ri)])
    ...
    for ji = 1:length(cell_struct(ri).Transformed_Dapi_Boundaries)
        ...
        for jid = 2:length(cell_struct(ri).Transformed_Dapi_Boundaries{ji,1})
            ...
            dapi_y_shift = [dapi_y_shift cell_struct(ri).Transformed_Dapi_Boundaries{ji,1}(jid,1)];
        end
    end
    
    dapi_x_shift = mean(dapi_x_shift);
    dapi_y_shift = mean(dapi_y_shift);
    %dapi_x_shift = 0;
    for ji = 1:length(cell_struct(ri).Transformed_Dapi_Boundaries)
        cell_struct(ri).Transformed_Dapi_Boundaries{ji,1}(:,2) = cell_struct(ri).Transformed_Dapi_Boundaries{ji,1}(:,2) - dapi_x_shift;
        cell_struct(ri).Transformed_Dapi_Boundaries{ji,1}(:,1) = cell_struct(ri).Transformed_Dapi_Boundaries{ji,1}(:,1) - dapi_y_shift;
    end
    ...
    cell_region_struct(rid).Pole_Volume = max([0,pole_membrane_volume]);
    cell_region_struct(rid).Middle_Volume = max([0,cell_region_struct(rid).Middle_Volume - pole_middle_volume]);

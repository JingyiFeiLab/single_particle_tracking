function dapi_objects = riboMask(stack_dapi)
...
int_thresh = 0.0001; % Intensity Threshold             <=="a relevant parameter out of two"  0.0001
                                            %(bigger is more generous, if less than 0.1, all are similar)
convolve_thresh = .05; % Threshold for Voxels to include in Convolved data  .05
shape2D_thresh = 3.5;  % <--- Splitting threshold, you can change this   3.5
shape3D_thresh = 2;
conc = .3;          % .3
background_thresh = .1;   %.1
pix_neigh = 7; % 7 is good
volume_thresh = 40 * (2*slices2D + 1);
slice_thresh = slices2D + 1;
zangle_thresh = 1;
dist_thresh = 4;  %4
low_pass_check = 1; % 1 = on. Change to 0 if you want to turn it off  1
gap_thresh = 5;   % 5

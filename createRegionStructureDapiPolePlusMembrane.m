...
membrane_definition = .3; %Distance to membrane (in microns) to be considered a membrane spot
...
for ri = 1:length(cell_struct)
    ...
    membrane_Volume = 2*((1-pole_definition)*cell_struct(ri).Cell_Y_Axis*(cell_struct(ri).Cell_X_Axis*membrane_pixels-membrane_pixels^2));
    cell_region_struct(rid).Membrane_Volume = membrane_Volume;
    cell_region_struct(rid).Pole_Plus_Membrane_Volume = pole_volume + membrane_Volume;
    ...
    cell_region_struct(rid).Pole_Plus_Membrane_Spots = pole + membrane;
    rid = rid + 1;
